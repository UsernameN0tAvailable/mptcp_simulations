...
NS_LOG_COMPONENT_DEFINE("MixedBulkSend");
...
int main(int argc, char *argv[]) {
     ...
     */
    LogComponentEnable("MixedBulkSend", LOG_ERROR );
    ...
    CommandLine cmd;
    cmd.AddValue("ber", "Bit Error Ratio at satellite links during off phase", ber);
    ...
    for(uint32_t index = 0; index < 4; index++){
        ...
        // set the application attributes and initialize application
        MpTcpBulkSendHelper source("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address(allIpv4Interfaces[connectionsCount - (index +1)].GetAddress(1)), mpTcpPort));
        ...
        // we initialize separate or uncoupled TCP connections at two terminals
        if(index % 2 == 0)
            source.SetAttribute("IsUncoupled", BooleanValue(true));

        // install the application to the terminal
        allSendApps[index] = source.Install(terminal1Nodes.Get(index));
        allSendApps[index].Start(Seconds(appStart));
        allSendApps[index].Stop(Seconds(appStop));
    }

    // Create and install packet sinks to terminals
     ...
     */
    uint32_t mptcpTotData = 0;
    double mptcpTotTime = 0;
    uint32_t tcpTotData = 0;
    double tcpTotTime = 0;
    ...
    // Compute Throughput and convert to bits from bytes
    double mptcpOutput = (mptcpTotData / (mptcpTotTime / 2)) * 8;
    double tcpOutput = (tcpTotData / (tcpTotTime / 2)) * 8;

    /*
     * Output
     */
    std::stringstream ss1;
    ss1 << mptcpOutput << "\t" << tcpOutput ;
